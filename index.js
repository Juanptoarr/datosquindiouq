...
// Leer el directorio "archivos" en el sistema de archivos
fs.readdir('archivos', (err, files) => {
  if (err) { // Si ocurre un error al leer el directorio
    console.error(err);
    return;
  }
  ...
  // Iterar sobre cada archivo en el directorio "archivos"
  files.forEach(file => {
    const filePath = path.join('archivos', file); // Obtener la ruta completa del archivo
